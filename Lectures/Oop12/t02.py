...
# клас домішок для класів Human та Car
class Logged:
    def log(self):
        if isinstance(self, Diagnosabe):
            print(f"Діагностуємо екземпляру класу {type(self)}")
            self.diagnose()

class CarLogged(Car, Logged):
    pass

class HumanLogged(Human, Logged):
    pass
